...
export default function PrescriptionUploadModal({ onClose, onSuccess, onError }: Props) {
  ...
  async function handleUpload() {
    ...
    try {
      ...
      const { data } = await api.post<PrescriptionOcrResponse>('/prescription-ocr/upload', formData, {
      ...
      if (resp?.status === 503) {
        ...
        onError(resp?.data?.detail ?? '업로드에 실패했습니다');
      }
    } finally {
      setUploading(false);
    }
  ...
  return (
    <Modal isOpen onClose={onClose} title="처방전 촬영/업로드">
