...
class TestReadRecentVisits:
    def test_grouping_by_preserial(self, reader):
        """Preserial별로 VisitRecord 생성."""
        r, mock_conn = reader
        cursor = MagicMock()
        mock_conn.cursor.return_value = cursor
        cursor.fetchall.return_value = [
            {
                "Preserial": "20260301000001",
                "CusCode": "P001",
                "Indate": "20260301",
                "Tdays": 7,
        ...
        result = r.read_recent_visits(date(2026, 3, 1))
        assert len(result) == 2
    ...
    def test_date_parsing(self, reader):
        ...
        result = r.read_recent_visits(date(2026, 12, 1))
    ...
    def test_invalid_date_skipped(self, reader):
        ...
        result = r.read_recent_visits(date(2026, 1, 1))
        assert len(result) == 0
